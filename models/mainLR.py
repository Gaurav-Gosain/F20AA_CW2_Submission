...
def prockfold(dfLematized, index, model, numberOfFolds, optionsName, nGram):
    ...
    with open(f"data/unbalanced_{model['model'].__class__.__name__}_{model['vectorizer'].__class__.__name__}_{optionsName}_{nGram}_confusion_matrix.npy", "wb") as of:
        np.save(of, confusion_matrix_result)
    # Save the file as json
    with open(f"data/unBalanced_{model['model'].__class__.__name__}_{model['vectorizer'].__class__.__name__}_{optionsName}_{nGram}.json", 'w') as f:
...
def main():
    ...
    p = multiprocessing.Process(target=prockfold, args=(
